...
export default function CreatureLibraryTable({
  rows,
  total,
  page,
  pageSize,
  ...
  rows: {id:number; name:string; created_at:string; size?:string; creature_type?:string; challenge_rating?:string}[] | null;
  total: number;
  page: number;
  pageSize: number;
  search: string;
  ...
  return (
    <div className="card-flex">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold mb-3">Creature Library</h2>
      ...
      {!slice || slice.length===0 ? (
        <p className="text-slate-300">No creatures saved yet.</p>
      ) : (
        <div className="table-wrap">
          <table className="table">
            ...
            <tbody>
              {slice.map(r => {
                const size = r.size || "";
                const type = r.creature_type || "";
                const cr = r.challenge_rating || "";
                const nameDisplay = size && type && cr 
                  ? `${r.name}: ${size} ${type} (CR ${cr})`
                  : r.name;
                return (
                  <tr key={r.id}>
                    <td>{nameDisplay}</td>
